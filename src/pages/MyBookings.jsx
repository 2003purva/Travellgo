...
const MyBookings = () => {
    ...
    const fetchMyBookings = async () => {
      try {
        ...
        const validBookings = (bookingsRes.data || []).filter(b => {
          ...
          const isHotelValid = hotelData?.hotelName && hotelData?.city;

          // Check Package Validity
          const packageData = (Array.isArray(b.packages) && b.packages.length > 0) ? b.packages[0] : b.packages;
          const isPackageValid = packageData?.packageName && packageData?.city;
        ...
        const otherBookings = validBookings.map(b => {
          ...
          //  Handle both Array (OneToMany) and Object (ManyToOne legacy) cases
          const hotelData = (Array.isArray(b.hotels) && b.hotels.length > 0) ? b.hotels[0] : b.hotel;
          const packageData = (Array.isArray(b.packages) && b.packages.length > 0) ? b.packages[0] : b.packages;
  ...
  const generateInvoice = (booking) => {
    ...
    doc.save(`Invoice_${booking.title.replaceAll(/\s+/g, '_')}.pdf`);
  };
                ...
                >
                  ...
                  <div className="card-media">
                    ...
                    <div className="type-badge">
                      {(() => {
                        if (booking.type === 'Bus') return <Ticket size={12} />;
                        if (booking.type === 'Hotel') return <HotelIcon size={12} />;
                  ...
                  <div className="card-content">
                    ...
                    <div className="card-body">
                      <p className="description-text">
                        {(() => {
                          if (!booking.description) return "Experience an unforgettable journey with TravelGO.";
                          return booking.description.length > 120 ? booking.description.substring(0, 120) + "..." : booking.description;
                        })()}
                      </p>
                      ...
                      {booking.amenities && booking.amenities.length > 0 && (
                        <div className="amenities-tags">
                          {booking.amenities.slice(0, 4).map((tag, idx) => (
                            <span key={tag.trim()} className="amenity-tag">{tag.trim()}</span>
                          ))}
